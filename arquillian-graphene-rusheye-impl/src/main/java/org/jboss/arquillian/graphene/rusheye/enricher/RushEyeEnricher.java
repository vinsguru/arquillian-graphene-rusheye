package org.jboss.arquillian.graphene.rusheye.enricher;
...
import org.boss.arquillian.graphene.rusheye.exception.RushEyeExtensionInitializationException;
...
import org.jboss.arquillian.graphene.rusheye.comparator.OcularImpl;
...
public class RushEyeEnricher implements TestEnricher {
    ...
    public void enrich(Object page) {
    	...
    	if(page.getClass().isAnnotationPresent(Snap.class)){
            ...
            float onePixelTreshold = page.getClass().getAnnotation(Snap.class).perceptionTreshold();
            ...
            for (Field field : fields) {
                try {
                    setFieldValue(page, field, new OcularImpl(isPageFragment, pattern, onePixelTreshold, masks));
                } catch (Exception e) {
